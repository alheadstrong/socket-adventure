    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        return ['You are in the room with white wallpaper',
                'You are in a giant, dark cave.',
                'You are in a brick-lined cylinder.',
                'You are on a balcony overlooking a vast chasm'][room_number]
    ...
    def route(self):
        ...
        if "say" in self.input_buffer:
            arg = self.input_buffer.replace('say ', '')
            self.say(arg)

        if "move" in self.input_buffer:
            arg = self.input_buffer.replace('move ', '')
            self.move(arg)
            self.room_description(self.room)
